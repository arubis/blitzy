defmodule Blitzy.Worker do
  ...
  def start(url, caller, func \\ &HTTPoison.get/1) do
    {timestamp, response} = Duration.measure(func, [url])
    send(caller, {self(), handle_response({Duration.to_milliseconds(timestamp), response})})
  end
  ...
  defp handle_response({_msecs, _}) do
    Logger.info "worker [#{node()}-#{inspect self()}] errored; reason unknown"
    {:error, :unknown}
  end
end
